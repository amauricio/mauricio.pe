;(function () {
  ...
  ;(function () {
	...
	if(page.length>0){
		page = page[0];
		var page_text = page.innerHTML;
		page.innerHTML = page_text.replace('Page', 'Página').replace('of','de');
	}
	var old = document.getElementsByClassName('older-posts');
	if(old.length>0)
	old[0].innerHTML = old[0].innerHTML.replace('Older Posts', 'Publicaciones antiguas');

	var ne = document.getElementsByClassName('newer-posts');
	if(ne.length>0){
		console.log(ne[0]);
		ne[0].innerHTML = ne[0].innerHTML.replace('Newer Posts', 'Publicaciones recientes');
	}
